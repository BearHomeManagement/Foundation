...
(() => {
  ...
  function dateLabel(d){return new Date(d+'T12:00:00').toLocaleDateString([],{weekday:'short',month:'short',day:'numeric'});}

  function scheduleContent(w,today){
    if(scheduleView==='day'){
      const jobs=w.filter(x=>x.scheduled_date===today);
      return jobs.slice(0,12).map(jobCard).join('')||'<div class="dashboard-empty">Nothing scheduled today.</div>';
    }
    if(scheduleView==='week'){
      const start=mondayOf(new Date());
      return `<div class="week-grid active">${Array.from({length:7},(_,i)=>{
        const d=new Date(start); d.setDate(start.getDate()+i); const key=iso(d);
        const jobs=w.filter(x=>x.scheduled_date===key);
        return `<div class="week-day"><h4>${dateLabel(key)}</h4>${jobs.map(jobCard).join('')||'<small class="muted">No jobs</small>'}</div>`;
      }).join('')}</div>`;
    }
    const now=new Date(), first=new Date(now.getFullYear(),now.getMonth(),1), last=new Date(now.getFullYear(),now.getMonth()+1,0);
    const offset=(first.getDay()+6)%7;
    const cells=[];
    for(let i=0;i<offset;i++) cells.push('<div class="day-cell"></div>');
    for(let day=1;day<=last.getDate();day++){
      const d=new Date(now.getFullYear(),now.getMonth(),day), key=iso(d), jobs=w.filter(x=>x.scheduled_date===key);
      cells.push(`<div class="day-cell"><div class="num">${day}</div>${jobs.slice(0,4).map(x=>`<div class="month-job ${jobClass(x)}">${esc(x.title||'Work Order')}</div>`).join('')}</div>`);
    }
    return `<div class="month-grid active"><div class="dow">Mon</div><div class="dow">Tue</div><div class="dow">Wed</div><div class="dow">Thu</div><div class="dow">Fri</div><div class="dow">Sat</div><div class="dow">Sun</div>${cells.join('')}</div>`;
  }
  ...
  function render(){
        ...
        <div>
          <div class="panel schedule-card">
            ...
            <div class="schedule-toolbar">
              ...
              <div class="date-control"><strong>${new Date().toLocaleDateString([],{month:'long',year:'numeric'})}</strong></div>
            </div>
            <div id="scheduleViewContent" class="content-list" style="padding:14px">${scheduleContent(w,today)}</div>
    ...
    el.querySelectorAll('[data-schedule-view]').forEach(btn=>{
      btn.onclick=()=>{scheduleView=btn.dataset.scheduleView;render();};
    });
